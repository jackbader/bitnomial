...
const PriceLadderInnerList: FC<PriceLadderInnerListProps> = (props) => {
    ...
    const [selectedIndex, setSelectedIndex] = useState(pricesToShow.indexOf(midPointPrice));
    ...
    return (
        <div className={styles.priceLadderContainer}>
            ...
            <div className={styles.priceLadderActionButtons}>
                <Button
                    variant="default"
                    disabled={orderBookPriceMap.size === 0}
                    onClick={clickedShouldShowAllPrices}
                    aria-label={shouldShowAllPrices ? 'Collapse' : 'Expand'}
                >
                    {shouldShowAllPrices ? 'Collapse' : 'Expand'}
                </Button>

                <Button
                    variant="default"
                    onClick={() => scrollToPrice(midPointPrice)}
                    aria-label="Center"
                >
                    CENTER
                </Button>
