...
const PriceLadderNew: FC<PriceLadderNewProps> = (props) => {
  ...
  useEffect(() => {
    ...
    if (allPrices.length === 0) {
      ...
      // get 20 prices above and below last traded price
      const range = 20000;
      const prices = [];
      for (
        let i = lastTradedPrice + range;
        i > lastTradedPrice - range - 1;
        i--
      ) {
        prices.push(i);
      }
  ...
  const midPointPrice = useMemo(() => {
    ...
    const bestBid = orderBookData.bids.sort((a, b) => b.price - a.price)[0]
      .price;
    // lowest price on the sell side.
    const bestAsk = orderBookData.asks.sort((a, b) => a.price - b.price)[0]
      .price;
  ...
  const pricesToShow = shouldShowAllPrices
    ? allPrices
    : allPrices.filter(
        (price) => orderBookPriceMap.has(price) || price === midPointPrice
      );
